...
public class PlayerAttack : MonoBehaviour
{
    ...
    [Tooltip("검 색상")]
    public Color swordColor = new Color(0.8f, 0.8f, 0.9f, 1f);
    ...
    void CreateSword()
    {
        // 이미 자식으로 PlayerSword가 있는지 확인
        Transform existingSword = transform.Find("PlayerSword");
        if (existingSword != null)
        {
            Debug.LogWarning($"[PlayerAttack] {gameObject.name}에 이미 PlayerSword가 있습니다. 재사용합니다.");
            swordObject = existingSword.gameObject;
            swordRenderer = swordObject.GetComponent<SpriteRenderer>();
            return;
        }
        
        swordObject = new GameObject("PlayerSword");
        ...
        SwordDamage swordDamage = swordObject.AddComponent<SwordDamage>();
